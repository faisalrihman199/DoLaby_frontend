...
export default function ChooseOutfitStep({ onTabChange, onDropItem, onImageUpload, onTryOn, gallery }) {
  ...
  return (
    <div className="rounded-[22px] bg-white p-4 md:p-5 shadow-sm ring-1 ring-[#cfe0f2]/70">
      ...
      {/* Try On Button with Sparkles */}
      {onTryOn && (selected.Top || selected.Bottom || selected.Shoes) && (
        <button
          onClick={onTryOn}
          className="mt-4 w-full flex items-center justify-center gap-2 bg-[#12396F] hover:bg-[#0e2d58] text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200"
        >
